...
\textbf{Anmeldung für das Proseminar}:
...
\end{bsp}
\begin{satz}[Majoranten und Minoranten]
...
Wir können also \( x\in\R \) mit \((x,0) \in\C \) identifizieren. \( \Rightarrow \R\supseteq \C \) und \(e_1=1\).\\
\( z=(x,y) = (x,0) + (0,y) = x(1,0) + y(0,1) = x e_1 + y e_2 \)
Sei \( z\in\C \), also \( z=(x,y) \), mit \(x,y \in\R \). Wir identifizieren geometrisch:
%BILD
