...
\begin{bew}\leavevmode
	\begin{enumerate}[(a)]
		\item Folgt direkt aus \hyperref[ax:A1]{(A1)} und der Definition von \(a > b \).
		\item \(a - c = \underbrace{(a - b)}_{> 0} + \underbrace{(b - c)}_{> 0} \overset{\text{(A2)}}{>} 0 \).
		\item Im Allgemeinen gilt \((a + c) - (b + c) = a - b > 0 \).
					\begin{faelle}
						\item[Fall \(c > 0 \):] \(ac - bc = \underbrace{(a - b)}_{> 0} \cdot c \overset{\text{(A2)}}{>}0 \).
						\item[Fall \(c < 0 \):] Es gilt \(\minus c > 0 \implies bc - ac = \underbrace{(a - b)}_{> 0} \cdot \underbrace{(\minus c)}_{> 0} \overset{\text{\hyperref[ax:A2]{(A2)}}}{>} 0 \).
					\end{faelle}
		\item \((a+c)-(b+d) = (a-b)+(c-d)>0 \) nach \hyperref[ax:A2]{(A2)}. \\
		...
		\item \begin{faelle}
						\item[Fall \(a > 0 \):] \(a^2 = a\cdot a > 0 \) \hyperref[ax:A2]{(A2)}.
						\item[Fall \(a < 0 \):] \(a^2 = (-a)\cdot(-a) > 0 \) \hyperref[ax:A2]{(A2)}.
			    \end{faelle}
		...
		\item \(a > b\ko  \; 0 > \lambda > 1 \implies \lambda > 0 \wedge 1 - \lambda > 0 \\
			    ...
			    Insbesondere gilt mit \(\lambda = \frac{1}{2} \) auch \(b < \frac{1}{2} b + \frac{1}{2}a = \frac{a + b}{2} < a \).
		\item Übung.\qedhere
	\end{enumerate}
\end{bew}
...
\begin{bew}\leavevmode
	\begin{enumerate}[(a)]
		...
		\item Sei \(x > 0 \overset{\autoref{satz:arechenregeln}{\text{(f)}}}{\implies} \frac{1}{x} > 0 \implies \, \exists n \in \N \colon \; n > \frac{1}{x} \\
		      \underset{\autoref{satz:arechenregeln}{\text{(g)}}}{\implies} x = \frac{1}{\frac{1}{x}} > \frac{1}{n} \).\qedhere
	\end{enumerate}
\end{bew}
