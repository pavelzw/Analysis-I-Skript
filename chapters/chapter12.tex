...
\end{bew}
\[ x\in\R^d, |x| := {\left( \sum_{j=1}^d x_j^2 \right)}^{1/2} \]
...
ist \( w\neq 0 \Rightarrow w_n \neq 0 \) für fast alle \(n \quad \bar{z_n} \rightarrow \bar{z} \) und 
\[ \frac{z_n}{w_n} \rightarrow \frac{z}{w}, n\rightarrow\infty. \]
\begin{bew}
	Wie im Reellen mit \gqq{Offensichtlicher Modifikation}. Ist \( (x_n)_n \) konv. Folge \( \Rightarrow |x_n| \) ist beschränkt.
\end{bew}
