...
\begin{bem}\leavevmode
	\begin{enumerate}[(a)]
		\item \((\K \ko +) \) und \((\K \setminus \set{0} \ko \cdot) \) sind kommutative Gruppen.
...
\begin{bew}\leavevmode
	\begin{enumerate}[(a)]
		\item \begin{enumerate}
				...
				\item[(v)] \(a\cdot 0 = a \cdot (0+0) =a \cdot 0 + a \cdot 0 = a \cdot 0 + a \cdot 0 \\
				...
				\item[(vi)] \(a \cdot b + a \cdot (\minus b) = a \cdot (b + (\minus b)) = a \cdot 0= 0 \\
							...
							Somit auch \((\minus a)(\minus b) = \minus((\minus a)b) = \minus(b(\minus a)) = \minus(\minus ba) = \minus(\minus ab) = ab \).
...
\begin{bew}\leavevmode
	\begin{enumerate}
		...
		\item[(d)] Nach \autoref{satz:arechenregeln} gilt \( \abs{a + b}^{2} = {(a+b)}^{2} = a^{2} + 2ab + b^{2} \\
				   ...
				   = \abs{a}^{2} + 2\abs{a}\abs{b} + \abs{b}^{2} = {(\abs{a} + \abs{b}\}^{2} \). \\
...
\begin{notation}
	...
	\begin{enumerate}[ ]
		...
		\item \(\Format{[a \ko b)} \coloneqq \set{x \in \R  \; \vert  \; a \leq x < b} \) nach rechts halboffenes Intervall.
		\item \(\Format{(a \ko b]} \coloneqq \set{x \in \R  \; \vert  \; a < x \leq b} \) nach links halboffenes Intervall.
		\item \(\begin{rcases}
...
\begin{bew} (Ohne vollständige Induktion)\leavevmode \\
	\[\begin{aligned}[t]
			...
			&\implies& x \cdot S_n & = \sum_{k = 0}^{n} x^{k + 1} = \sum_{j = 1}^{n + 1} x^j \\
			&\implies& (1 - x)S_n  & = S_n - x S_n = \sum_{k = 0}^{n} x^k - \sum_{j = 1}^{n + 1} x^j = x^0 - x^{n + 1} = 1 - x^{n + 1} \\
