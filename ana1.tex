...
\begin{bew}
	...
	$$\overset{\text{Satz 5.1.2}}{\Rightarrow}\exists n_* = \min A \in \mathbb{N}$$ 
	Sei $\frac{m}{n_*} = \sqrt{k}$, dann gilt
	$$m-n_* = \underbrace{(\sqrt{k}-1)}_{<1} n_*$$ %DURCHGESTRICHEN
	Aber wähle $q\in\mathbb{N}: q\leq \sqrt{k} < q+1$\\
	Existiert nach Lemma 5.2.1. Da $\sqrt{k} \notin \mathbb{N}$ gilt $q<\sqrt{k}<q+1$.\\
	Also gilt:
	...
	Somit muss $\sqrt{k}\in\mathbb{R}\setminus\mathbb{Q}$ sein.
\end{bew}
\end{document}