	...
	\begin{enumerate}[(1)]
		...
		\item (Geometrische Folge)
			  ...
			  \begin{bew}
				...
				Aus \autoref{satz:bern_ungl} folgt
				\[ {(1+x)}^n \geq 1 + nx \ko \]
				und somit
				\[ \abs{q^n-0} = \abs{q^n} = \abs{q}^n = \frac{1}{{(1+x)}^n} \leq \frac{1}{1+nx}. \]
				Sei nun \(\varepsilon > 0 \) und wähle \(k \in \N \) mit \(kx > \frac{1}{\varepsilon} \). Dann gilt
				\[\abs{q^n-0} \leq \frac{1}{1+nx} \leq \frac{1}{nx} < \varepsilon \]
				für alle \( n\geq k \).
		  	  \end{bew}
		\item Sei \(a \in \R \) mit \(a > 0 \). Dann gilt für \(a_n \coloneqq a^{\frac{1}{n}} \ko \, \limes{n}a_n = 1 \).
			  \begin{bew}
				\begin{faelle}
					\item[Fall \(a = 1 \):] \(1^{\frac{1}{n}} = 1 \convtoinf{n} 1 \).
					\item[Fall \(a > 1 \):] Es ist \(a_n = a^{\frac{1}{n}}>1 \) und somit \(q_n \coloneqq a_n-1 = a^{\frac{1}{n}} -1 >0 \).
						Damit gilt
						\[a_n = a^{\frac{1}{n}} = 1+q_n \Rightarrow a = {(1+q_n)}^n 
						\underset{\text{\hyperref[satz:bern_ungl]{Bern.\ Ungl.}}}{\geq} 1+ nq_n \]
						\(\implies 0 \leq q_n \leq \frac{a-1}{n} \) für alle \(n \in \N \).
						Zu \(\varepsilon > 0 \) wähle \(k \in \N \) mit \(k > \frac{a-1}{\varepsilon} \). Dann gilt für \(n \geq k \)
						\[\abs{a_n-1} = \abs{a^{\frac{1}{n}}-1}= a^{\frac{1}{n}} -1 = q_n \leq \frac{a-1}{n} < \varepsilon. \]
					\item[Fall \(0 < a < 1 \):] Es ist \(b \coloneqq \frac{1}{a}>1 \). Also \( \limes{n}b^{\frac{1}{n}} = 1 \). Somit gilt
						\begin{align*}
							\abs{a^{\frac{1}{n}} - 1} =& a^{\frac{1}{n}} \abs{1 - \frac{a}{a^{\frac{1}{n}}}} 
							= a^{\frac{1}{n}} \abs{1 - {\left(\frac{1}{a}\right)}^{\frac{1}{n}}}\\
							=& a^{\frac{1}{n}} \abs{1-b^{\frac{1}{n}}}
							\leq \abs{1-b^{\frac{1}{n}}} \convtoinf{n} 0.
						\end{align*}
						\(\implies \limes{n} a^{\frac{1}{n}} = 1 \). \qedhere
				\end{faelle}
			  \end{bew}
		\item Es gilt \(\limes{n}n^{\frac{1}{n}}= \limes{n}\sqrt[n]{n} = 1 \).
			  \begin{bew}
				Es gilt für \(q \geq 0 \)
				\begin{align*}
					{(1+q)}^n =& \sum_{k=0}^{n} \binom{n}{k} q^k 1^{n-k} \\
							  =& 1 + \binom{n}{1}q + \binom{n}{2} q^2 + \sum_{k=3}^{n}\binom{n}{k} q^k 1^{n-k} \\
