...
\begin{defi*}
	...
	In diesem Fall sage auch \gqq{Die Kardinalität von \(A \) ist kleiner als die Kardinalität von \(B \)} oder
	\gqq{\(A \) ist weniger Mächtig als B}.
\end{defi*}
...
\begin{bew}\leavevmode
	\induktion{
		...
		schreiben. z.B. \(S_{3 \ko 2} = \set{(1 \ko 3 \ko 2) \ko (2 \ko 3 \ko 1)} \). \\
	...
	\induktion{
		...
		\(\begin{aligned}
			\#\set{A \subseteq [n+1] \; \vert \; \#A = k}&=\phantom{+} \#\set{A \subseteq [n] \; \vert \; \#A = k} \\
					   	 ...
					   	 \overset{\autoref{satz:bin_rek}}&{=} \binom{n+1}{k}.
		  \end{aligned}\)
	}
\end{bew}
