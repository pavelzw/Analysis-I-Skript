...
\begin{bew}
	...
	\[e-e_n = \limes{n}\left( \sum_{k=0}^{m} \frac{1}{k!} - \sum_{k=0}^{n} \frac{1}{k!} \right) = \limes{m} \left(\sum_{k=n+1}^{m} \frac{1}{k!} \right) \]
	\begin{align*}
	 	(*)\quad k \geq n+1\\
...
\begin{satz}[Invervallschachtelungsprinzip]
	...
	\begin{center}
		\begin{tikzpicture}
			...
			\draw (3,0) node {\(]\)} node[below=2mm] {\(b_3\)} node[above=2mm] {\(I_3\)};
			\draw (5,0) node {\(]\)} node[below=2mm] {\(b_2\)} node[above=2mm] {\(I_2\)};
			\draw (6,0) node {\(]\)} node[below=2mm] {\(b_1\)} node[above=2mm] {\(I_1\)};
		\end{tikzpicture}
	\end{center}
\end{satz}
\begin{bew}\leavevmode
	\begin{enumerate}
		...
		Ang. \( \exists a,\tilde{a}\in \bigcap_{n\in\N} I_n \quad a\neq \tilde{a} \) (o.B.d.A. \(\tilde{a}>a\)).\\
...
\begin{bsp}
	...
	\begin{align*}
		...
		&\leq 1- {\left(\frac{1}{k}\right)}^n + \frac{1}{k^n} = 1.\\
		\overundersett{Mon.}{Konv.}{\Rightarrow} a &= \limes{n} a_n \text{ existiert.}\\
	\end{align*}
	Beachte: \(0 \leq x-a_n < \frac{1}{k^n} \overset{n \rightarrow \infty}\rightarrow 0\) \\	 
	\begin{align*}
