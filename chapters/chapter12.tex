...
\begin{satz}[Cauchy-Schwarz-Ungleichung, CSU]
	\[ \forall x,y\in\R^d: \abs{\langle x,y\rangle}\leq \abs{x}\abs{y}\]
	und \gqq{\(=\)} gilt, \( \Leftrightarrow x,y \) sind linear abhängig.
\end{satz}
\begin{bew}
...
\begin{defi}[komplexe Konjugation]
	Für eine komplexe Zahl \( z=x+iy, \; x,y\in\R \) definiert man die konjugierte komplexe Zahl durch
	\[ \bar{z} := x - iy \]
...
\end{satz}
\begin{bew}
	\gqq{\(\Rightarrow \)}: Es gilt \( \limes{n}x_n = a \).\\
