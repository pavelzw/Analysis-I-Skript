...
\begin{satz*}[Schubfachprinzip]\label{satz:schubfach}\leavevmode \\
	...
	\(\#\inverse{f}(\set{k}) \geq 2\).
\end{satz*}
\begin{bew}
	Angenommen für alle \(k \in [n] \) gilt \(\#\inverse{f}(\set{k}) < 2 \). Es gilt also für alle \(k \)\\
	\(\inverse{f}(\set{k}) = \set{j_k} \) für ein \(j_k \in [m]\). Ferner gilt \(\#[n] = n\).\\
	Somit folgt, dass \(\#\inverse{f}([n]) = n < m \). \Lightning{ zu \(m > n\).}
\end{bew}
...
\begin{kor} \leavevmode \\
	Sind \(n \ko m \in\N \) und ist \(f \colon [m] \longrightarrow[n] \) bijektiv \(\implies m = n \).
\end{kor}
\begin{bew}
...
\begin{defi}
	...
	%Die Anzahl der Elemente von \(M \) (\( \#M \)) ist dann \( \#M\coloneqqn \), setzen \( \#\emptyset \coloneqq 0 \). Eine Menge ist unendlich, falls sie nicht endlich ist.
	...
	\begin{bem}
		...
		\( h\coloneqq f^{-1}\circ g = [m]\rightarrow[n] \) ist auch bijektiv. \(\overset{\text{Korr. 2}}{\Rightarrow} m=n \).
	\end{bem}
