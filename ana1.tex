...
\end{bsp}
\begin{bsp}
	\[\sum_{n=2}^{\infty}\frac{1}{n(n-1)}=1 \text{ (telekopierende Summe)} \]
\end{bsp}
\begin{bew}
	\begin{align*}
...
\begin{bew}
	...
	Monotone Konvergenz, da \((S_n)_n\) konvergiert, also konvergiert \(\sum_{j=1}^{\infty} \frac{1}{j^2} = 2 - \frac{1}{n} \leq 2 \forall n \).
\end{bew}
\begin{kor}
...
\end{satz}
\begin{bew}
	\[ S_n = \sum_{j=1}^{\infty}a_j, t_n = \sum_{j=1}^{\infty}b_j, S = \limes{n}S_n, t = \limes{n}t_n \]
	\[ \sum_{j=1}^{\infty}(\lambda a_n + \mu b_n) = \lambda\sum_{j=1}^{\infty}a_j + \mu \sum_{j=1}^{\infty}b_j = \lambda S_n + \mu t_n \rightarrow \lambda S + \mu t, n\rightarrow\infty. \]
\end{bew}
...
\end{satz}
\begin{bew}
	\[ S_n := \sum_{l=0}^{n}a_l, t := \sum_{\nu}^{n} 2^\nu a_{2^\nu} \]
	Haben: \( a_n\geq a_{n+1} \geq\dots\geq 0 \)\\
	Ist \(n \leq 2^K \), dann folgt
	\begin{align*}
		S_n &= a_0 + a_1 + a_2 + \dots + a_n\\
		&\leq a_0 + a_1 + a_2 + \dots + (a_{2^{k-1}+1} + \dots + a_{2^k})\\
		&= a_0 + (a_1 + a_2) + \dots + (a_{2^{k-1}+1} + \dots + a_{2^k})\\
		&\leq a_0 + 2a_1 + 4a_2 + \dots + \underbrace{(2^k - 2^{k-1})}_{=2^{k-1}}a^{2^{k-1}} = t_{k-1}
		% vermtl. irrelevant, Hundertmark hat sich verschrieben
		% &\leq a_0 + 2a_1 + 4a_3 + 8a_7 + \dots + (2^k - 2^{k-1})a_{2^{k-1}+1}\\
		% &\leq a_0 + 2a_1 + 4a_2
	\end{align*}
	\( \Rightarrow \) Für \(n \leq 2^k \) folgt 
	\[ S_n \leq t_{k-1} \]
	\( \overset{\text{Mon. Konv.}}{\Rightarrow} S_n \) konv., falls \( (t_k)_k \)  konvergent, d. h. falls \( \sum_{k=0}^{\infty}2^k a_{2^k} \) konvergent.\\
	Umgekehrt, ist \(n>2^k\)\\
	\begin{align*}
		S_n &\geq a_0 + a_1 + a_2 + (a_3 + a_4) + \dots + a_{2^k}\\
		&= a_0 + a_1 + a_2 + \underbrace{(a_3+a_4)}_{\geq 2a_4} + \dots + \underbrace{(a_{2^{k-1}+1}) + \dots + a_{2^k})}_{\geq (2^k - 2^{k-1})a_{2^k} }\\
		S_n &\geq a_0 + a_1 + a_2 + 2a_4 + \dots + 2^{k-1}a_{2^k}\\
		&\geq \frac{1}{2} \underbrace{(a_1 + 2a_2 + 4a_4 + \dots + 2^k a_{2^k})}_{=t_k}\\
		&\geq \frac{1}{2} t_k\\
		\Rightarrow \forall n>2^k \text{ folgt } t_k \leq 2 S_n
	\end{align*}
	Falls \(S_n \rightarrow S \)
	\[S_n \leq S_{n+1} \leq \dots \leq S. \]
	\[ \Rightarrow t_k \leq 2S \quad\forall k \]
	\[ \overset{\text{Mon. Konv.}}{\Rightarrow} t= \lim t_k \text{ existiert.} \]
\end{bew}
Anwendung:\\
Sei \(\alpha > 0\). \( \sum_{n=1}^{\infty}\frac{1}{n^\alpha} \) konvergent?\\
\( a_n = \frac{1}{n^\alpha} \) mon. fallende Nullfolge
\begin{align*}
	&\overset{\text{Satz 8}}{\Rightarrow} \sum_{n=1}^{\infty} \frac{1}{n^\alpha} \text{ konvergent}.\\
	&\Leftrightarrow \sum_{n=0}^{\infty}2^n \frac{1}{(2^n)^\alpha} \text{ konvergent}.\\
	&\Leftrightarrow \sum_{n=0}^{\infty}(2^{1-\alpha})^n \text{ konvergent}.\\
	&\Leftrightarrow\sum_{n=1}^{\infty}\frac{1}{n^\alpha} \text{ konvergent}. \Leftrightarrow \alpha >1
\end{align*}
%11.12.2018
%HUNDERTMARK HAT CAUCHYSCHER VERDICHTUGNSSATZ VERKACKT, JETZT NEU
...
\begin{bew}
	...
	\end{align*}
	\[ \text{und } 0 \leq S_{2n} = \sum_{j=1}^{2n} (-1)^{j+1} \]
	%WEITERMACHEN
	\( \overset{\text{Mon. Konv.}}{\Rightarrow} S_1 = \limes{n}S_{2n} \) existiert.\\
	\( S_2 = \limes{n}S_{2n+1} \) existiert.\\
	und: 
	...
	\[ \Rightarrow S_1 = S_2 \Rightarrow (S_n)_n \text{ konvergiert auch gegen } S_1. \]
\end{bew}
\begin{defi}
