...
\begin{lem}
    ...
    ein \( \delta > 0 \), sodass 
    \[ \forall \, x\in [a,b] \cap \bar{B_\delta(c)} : 
    f(x) \geq \frac{f(c)}{2} \]
\end{lem}
\begin{bew}
    Wegen \( \varepsilon \)-\(\delta \)-Char.\ der Stetigkeit
    folgt \( \varepsilon := \frac{f(c)}{2} > 0 \) die 
    Existenz von \( \delta > 0 \)
    \[ \forall \, x\in [a,b] \cap  \]%NOCH FERTIGMACHEN
\end{bew}
\end{document}